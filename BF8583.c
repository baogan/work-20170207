...
#define MY_BITGET(p,n) ((p)[(n-1)/8]&(0x80>>((n-1)%8)))       //Âèñbit‰Ω?
#define MY_BITSET(p,n) ((p)[(n-1)/8]|=(0x80>>((n-1)%8)))      //ËÆæbit‰Ω?
#define MY_BITCLR(p,n) ((p)[(n-1)/8]&=(~(0x80>>((n-1)%8))))   //Ê∏Öbit‰Ω?
...
int  SetField(int FieldNum,uchar *Buff,int DataLen)
{
	...
	if(FieldNum==1)//‰∏çÂÖÅËÆ∏Áõ¥Êé•‰øÆÊîπbitmap
		return -1;
	...
	if(MY_BITGET(GBitMap,FieldNum))//Âà†Èô§ÂéüÊù•ÁöÑÈÇ£‰∏™Âüü
	{
...
int MakeRetrunPacket(uchar *msg, uchar *RecvBuff, int RecvBuffLen,uchar *SendBuff,int *SendBuffLen)
{
	...
	{
		//«©µΩ
		SetPacket((uchar *)"\x08\x10\x00\x00\x00\x00\x00\x00\x00\x00",10);//1°¢2”Ú
	...
	{
		//”‡∂Ó
		SetPacket((uchar *)"\x02\x10\x00\x00\x00\x00\x00\x00\x00\x00",10);//1°¢2”Ú
	...
	{
		//œ˚∑—≥∑œ˚
		SetPacket((uchar *)"\x02\x30\x00\x00\x00\x00\x00\x00\x00\x00",10);//1°¢2”Ú
		if(fieldlen[2]==8)
			SetField(2,field[2],16);
		else if(fieldlen[2]==9)
			SetField(2,field[2],18);
